...
Write-Host -ForegroundColor Cyan -BackgroundColor DarkBlue "=-=-=" -NoNewLine;Write-Host -ForegroundColor Yellow " Gitをダウンロード・インストール " -NoNewLine;Write-Host -ForegroundColor Cyan -BackgroundColor DarkBlue "=-=-="
...
function Get-InstalledVersion {
    ...
    # 実行ファイルのバージョン情報を取得する
    $VersionInfo = [Version](Get-ItemProperty $CheckTargetFilePath).VersionInfo.FileVersionRaw
...
function Download-InstallArtifact {
    ...
    # GitのURL専用のバージョン表記
    $MajorVersion = $TargetVersion.Major
    $MinorVersion = $TargetVersion.Minor
    $BuildNumber = $TargetVersion.Build
    $RevisionNumber = $TargetVersion.Revision
    if ($RevisionNumber -eq -1) {
        $RevisionNumber = 1
    }
    $VersionForGit = [String]::Join(".", $MajorVersion, $MinorVersion, $BuildNumber, "windows", $RevisionNumber)
    ...
    $InstallArtifactDownloadURL = $InstallArtifactDownloadURL.Replace("{VersionForGit}", $VersionForGit)
...
function Do-AfterInstall {
    ...
    # 環境変数PATHに追加
    Add-Path (Join-Path $InstallDestinationFolderPath "cmd") "git.exe"
    Add-Path (Join-Path $InstallDestinationFolderPath "bin") "git.exe"
    Add-Path $InstallDestinationFolderPath "git-bash.exe"

    # Git-Bash を起動
    Write-Log "情報: Git-Bashを起動します。"
    Start-Sleep -Milliseconds 700
    Write-Log "情報: Git-Bashは起動後、数秒後に自動的に閉じます。"
    Start-Sleep -Milliseconds 700
    Write-Log "情報: Git-Bashの起動後は特に操作は行わないでください。"
    Start-Sleep -Milliseconds 1000
    Push-Location $InstallDestinationFolderPath
    try {
        $ProcessInfo = Start-Process (Join-Path $InstallDestinationFolderPath "bin\bash.exe") "-c 'sleep 3; exit'" -Verb RunAs -PassThru
        Write-Log "情報: Git-Bashの終了待機中..."
        Start-Sleep -Milliseconds 2000
        $ProcessInfo.WaitForExit()
        Start-Sleep -Milliseconds 500
    } finally {
        Pop-Location
        Write-Log "情報: Git-Bashの起動確認が完了しました。"
        Start-Sleep -Milliseconds 700
    }
}
