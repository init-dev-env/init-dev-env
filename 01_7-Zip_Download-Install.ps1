...
Write-Host -ForegroundColor Cyan -BackgroundColor DarkBlue "=-=-=" -NoNewLine;Write-Host -ForegroundColor Yellow " 7-Zipをダウンロード・インストール   " -NoNewLine;Write-Host -ForegroundColor Cyan -BackgroundColor DarkBlue "=-=-="
...
function Do-AfterInstall {
    ...
    # アプリケーションを指すレジストリキーの存在チェック
    if (-not (Test-Path "HKCU:\Software\7-Zip")) {
        Write-Log "エラー: インストール後のアプリケーションのレジストリキーが存在しません。インストールに失敗した可能性があります。[$("HKCU:\Software\7-Zip".Replace("HKCU:", "HKEY_CURRENT_USER"))]"
        Start-Sleep -Milliseconds 1250
        End-ThisScriptAsFailure
    }

    # Optionsサブキーがなければ作成する
    if (-not (Test-Path "HKCU:\Software\7-Zip\Options")) {
        New-Item "HKCU:\Software\7-Zip\Options" -Force | Out-Null
        Write-Log "情報: レジストリキーを作成しました。[$("HKCU:\Software\7-Zip\Options".Replace("HKCU:", "HKEY_CURRENT_USER"))]"
    ...
    if (-not $OptionsKey.Property.Contains("ContextMenu")) {
        ...
        Write-Log "情報: レジストリキー[$("HKCU:\Software\7-Zip\Options".Replace("HKCU:", "HKEY_CURRENT_USER"))]のエントリ[ContextMenu]を値[4967]で作成しました。"
        Start-Sleep -Milliseconds 700
    } else {
        # ContextMenuエントリの値を取得
        ...
        if ($ContextMenuValue -ne $ContextMenuMaskedValue) {
            Write-Log "警告: レジストリキー[$("HKCU:\Software\7-Zip\Options".Replace("HKCU:", "HKEY_CURRENT_USER"))]のエントリ[ContextMenu]の値が[$ContextMenuMaskedValue]ではありません。(値:[$ContextMenuValue])"
            Start-Sleep -Milliseconds 1250
            # ContextMenuエントリの値を更新する
            Set-ItemProperty -Path "HKCU:\Software\7-Zip\Options" -Name "ContextMenu" -Value $ContextMenuMaskedValue
            Write-Log "情報: レジストリキー[$("HKCU:\Software\7-Zip\Options".Replace("HKCU:", "HKEY_CURRENT_USER"))]のエントリ[ContextMenu]の値を[$ContextMenuMaskedValue]で更新しました。"
            Start-Sleep -Milliseconds 1250
        }
    }

    # MenuIconsエントリがなければ作成する
    if (-not $OptionsKey.Property.Contains("MenuIcons")) {
        ...
        Write-Log "情報: レジストリキー[$("HKCU:\Software\7-Zip\Options".Replace("HKCU:", "HKEY_CURRENT_USER"))]のエントリ[MenuIcons]を値[1]で作成しました。"
        Start-Sleep -Milliseconds 700
    } else {
        # MenuIconsエントリの値を取得
        $MenuIconsValue = Get-ItemPropertyValue $OptionsKey.PSPath -Name "MenuIcons"
        if ($MenuIconsValue -ne 1) {
            Write-Log "警告: レジストリキー[$("HKCU:\Software\7-Zip\Options".Replace("HKCU:", "HKEY_CURRENT_USER"))]のエントリ[MenuIcons]の値が[1]ではありません。(値:[$MenuIconsValue])"
            Start-Sleep -Milliseconds 1250
            # MenuIconsエントリの値を更新する
            Set-ItemProperty -Path "HKCU:\Software\7-Zip\Options" -Name "MenuIcons" -Value 1
            Write-Log "情報: レジストリキー[$("HKCU:\Software\7-Zip\Options".Replace("HKCU:", "HKEY_CURRENT_USER"))]のエントリ[MenuIcons]の値を[1]で更新しました。"
            Start-Sleep -Milliseconds 1250
        }
    }

    # Extractionサブキーがなければ作成する
    if (-not (Test-Path "HKCU:\Software\7-Zip\Extraction")) {
        New-Item "HKCU:\Software\7-Zip\Extraction" -Force | Out-Null
        Write-Log "情報: レジストリキーを作成しました。[$("HKCU:\Software\7-Zip\Extraction".Replace("HKCU:", "HKEY_CURRENT_USER"))]"
        Start-Sleep -Milliseconds 700
    }
    ...
    if (-not $ExtractionKey.Property.Contains("ShowPassword")) {
        ...
        Write-Log "情報: レジストリキー[$("HKCU:\Software\7-Zip\Extraction".Replace("HKCU:", "HKEY_CURRENT_USER"))]のエントリ[ShowPassword]を値[1]で作成しました。"
        Start-Sleep -Milliseconds 700
    } else {
        # ShowPasswordエントリの値を取得
        $ShowPasswordValue = Get-ItemPropertyValue $ExtractionKey.PSPath -Name "ShowPassword"
        if ($ShowPasswordValue -ne 1) {
            Write-Log "警告: レジストリキー[$("HKCU:\Software\7-Zip\Extraction".Replace("HKCU:", "HKEY_CURRENT_USER"))]のエントリ[ShowPassword]の値が[1]ではありません。(値:[$ShowPasswordValue])"
            Start-Sleep -Milliseconds 1250
            # ShowPasswordエントリの値を更新する
            Set-ItemProperty -Path "HKCU:\Software\7-Zip\Extraction" -Name "ShowPassword" -Value 1
            Write-Log "情報: レジストリキー[$("HKCU:\Software\7-Zip\Extraction".Replace("HKCU:", "HKEY_CURRENT_USER"))]のエントリ[ShowPassword]の値を[1]で更新しました。"
            Start-Sleep -Milliseconds 1250
        }
    }
    ...
    foreach ($ExtensionInfo in $ExtensionInfoList) {
        ...
        # 拡張子ごとのキーから参照されるキーがなければ作成する
        if (-not (Test-Path "HKCU:\Software\Classes\7-Zip.$Extension")) {
            New-Item "HKCU:\Software\Classes\7-Zip.$Extension" -Force | Out-Null
            Write-Log "情報: レジストリキーを作成しました。[$("HKCU:\Software\Classes\7-Zip.$Extension".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]"
            Start-Sleep -Milliseconds 700
        }

        # 拡張子ごとのキーから参照されるキーを取得する
        $CalleeKey = Get-Item "HKCU:\Software\Classes\7-Zip.$Extension"

        # 既定のエントリ(ファイルの種類の名前)がないかチェックし、なければ値を設定する
        if (-not $CalleeKey.Property.Contains("(default)")) {
            New-ItemProperty -Path "HKCU:\Software\Classes\7-Zip.$Extension" -Name "(default)" -PropertyType String -Value "$Extension アーカイブファイル" -Force | Out-Null
            Write-Log "情報: レジストリキー[$("HKCU:\Software\Classes\7-Zip.$Extension".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]の既定のエントリ[(default)]を値[$Extension アーカイブファイル]で更新しました。"
            Start-Sleep -Milliseconds 700
        } else {
            # 既定のエントリの値を取得
            $CalleeValue = Get-ItemPropertyValue $CalleeKey.PSPath -Name "(default)"
            if ($CalleeValue -ne "$Extension アーカイブファイル") {
                Write-Log "警告: レジストリキー[$("HKCU:\Software\Classes\7-Zip.$Extension".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]の既定のエントリ[(default)]の値が[$Extension アーカイブファイル]ではありません。(値:[$CalleeValue])"
                Start-Sleep -Milliseconds 1250
                # 既定のエントリの値を更新する
                Set-ItemProperty -Path "HKCU:\Software\Classes\7-Zip.$Extension" -Name "(default)" -Value "$Extension アーカイブファイル"
                Write-Log "情報: レジストリキー[$("HKCU:\Software\Classes\7-Zip.$Extension".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]の既定のエントリ[(default)]の値を[$Extension アーカイブファイル]で更新しました。"
                Start-Sleep -Milliseconds 1250
            }
        }

        # 拡張子ごとのアイコンを指定するキーがなければ作成する
        if (-not (Test-Path "HKCU:\Software\Classes\7-Zip.$Extension\DefaultIcon")) {
            ...
            Write-Log "情報: レジストリキーを作成しました。[$("HKCU:\Software\Classes\7-Zip.$Extension\DefaultIcon".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]"
            Start-Sleep -Milliseconds 700
        }

        # 拡張子ごとのアイコンを指定するキーを取得する
        $DefaultIconKey = Get-Item "HKCU:\Software\Classes\7-Zip.$Extension\DefaultIcon"

        # 既定のエントリ(アイコンリソース取得元ファイルパス、アイコンインデックス)がないかチェックし、なければ値を設定する
        if (-not $DefaultIconKey.Property.Contains("(default)")) {
            ...
            Write-Log "情報: レジストリキー[$("HKCU:\Software\Classes\7-Zip.$Extension\DefaultIcon".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]の既定のエントリ[(default)]を値[$InstallDestinationFolderPath\7z.dll,$IconIndex]で更新しました。"
            Start-Sleep -Milliseconds 700
        } else {
            # 既定のエントリの値を取得
            ...
            if ($DefaultIconValue -ne "$InstallDestinationFolderPath\7z.dll,$IconIndex") {
                Write-Log "警告: レジストリキー[$("HKCU:\Software\Classes\7-Zip.$Extension\DefaultIcon".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]の既定のエントリ[(default)]の値が[$InstallDestinationFolderPath\7z.dll,$IconIndex]ではありません。(値:[$DefaultIconValue])"
                Start-Sleep -Milliseconds 1250
                # 既定のエントリの値を更新する
                ...
                Write-Log "情報: レジストリキー[$("HKCU:\Software\Classes\7-Zip.$Extension\DefaultIcon".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]の既定のエントリ[(default)]の値を[$InstallDestinationFolderPath\7z.dll,$IconIndex]で更新しました。"
                Start-Sleep -Milliseconds 1250
            }
        }
        ...
        if (-not (Test-Path "HKCU:\Software\Classes\7-Zip.$Extension\Shell")) {
            ...
            Write-Log "情報: レジストリキーを作成しました。[$("HKCU:\Software\Classes\7-Zip.$Extension\Shell".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]"
            Start-Sleep -Milliseconds 700
        }

        # Shell\Openキーがなければ作成する
        if (-not (Test-Path "HKCU:\Software\Classes\7-Zip.$Extension\Shell\Open")) {
            ...
            Write-Log "情報: レジストリキーを作成しました。[$("HKCU:\Software\Classes\7-Zip.$Extension\Shell\Open".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]"
            Start-Sleep -Milliseconds 700
        }

        # Shell\Open\Commandキーがなければ作成する
        if (-not (Test-Path "HKCU:\Software\Classes\7-Zip.$Extension\Shell\Open\Command")) {
            ...
            Write-Log "情報: レジストリキーを作成しました。[$("HKCU:\Software\Classes\7-Zip.$Extension\Shell\Open\Command".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]"
            Start-Sleep -Milliseconds 700
        }

        # Shell\Open\Commandキーを取得する
        $CommandKey = Get-Item "HKCU:\Software\Classes\7-Zip.$Extension\Shell\Open\Command"

        # 既定のエントリ(実行コマンド)がないかチェックし、なければ値を設定する
        if (-not $CommandKey.Property.Contains("(default)")) {
            ...
            Write-Log "情報: レジストリキー[$("HKCU:\Software\Classes\7-Zip.$Extension\Shell\Open\Command".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]の既定のエントリ[(default)]を値[`"$InstallDestinationFolderPath\7zFM.exe`" `"%1`"]で更新しました。"
            Start-Sleep -Milliseconds 700
        }
        ...
        if (-not (Test-Path "HKCU:\Software\Classes\.$Extension")) {
            ...
            Write-Log "情報: レジストリキーを作成しました。[$("HKCU:\Software\Classes\.$Extension".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]"
            Start-Sleep -Milliseconds 700
        }
        ...
        if (-not $CallerKey.Property.Contains("(default)")) {
            ...
            Write-Log "情報: レジストリキー[$("HKCU:\Software\Classes\.$Extension".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]の既定のエントリ[(default)]を値[7-Zip.$Extension]で更新しました。"
            Start-Sleep -Milliseconds 700
        } else {
            # 既定のエントリの値を取得
            $CallerValue = Get-ItemPropertyValue $CallerKey.PSPath -Name "(default)"
            if ($CallerValue -ne "7-Zip.$Extension") {
                Write-Log "警告: レジストリキー[$("HKCU:\Software\Classes\.$Extension".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]の既定のエントリ[(default)]の値が[7-Zip.$Extension]ではありません。(値:[$CallerValue])"
                Start-Sleep -Milliseconds 1250
                # 既定のエントリの値を更新する
                Set-ItemProperty -Path "HKCU:\Software\Classes\.$Extension" -Name "(default)" -Value "7-Zip.$Extension"
                Write-Log "情報: レジストリキー[$("HKCU:\Software\Classes\.$Extension".Replace("HKCU:\Software\Classes", "HKEY_CLASSES_ROOT"))]の既定のエントリ[(default)]の値を[7-Zip.$Extension]で更新しました。"
                Start-Sleep -Milliseconds 1250
            }
        }
    }
