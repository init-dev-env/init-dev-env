...
Write-Host -ForegroundColor Cyan -BackgroundColor DarkBlue "=-=-=" -NoNewLine;Write-Host -ForegroundColor Yellow " DimScreenをダウンロード・インストール   " -NoNewLine;Write-Host -ForegroundColor Cyan -BackgroundColor DarkBlue "=-=-="
...
function Install-Application {
    ...
    # 実行中のプロセスが存在すれば終了させる
    (Get-Process -Name DimScreen -ErrorAction SilentlyContinue) | ForEach-Object { Stop-Process $_ -Force }
...
function Do-AfterInstall {
    ...
    # iniファイル初期生成
    $DimScreenIniFilePath = Join-Path $InstallDestinationFolderPath "DimScreen.ini"
    $DimScreenIniFileLines = [List[String]]::New()
    # ないと思うが一応既存ファイルチェック
    if (Test-Path $DimScreenIniFilePath -PathType Leaf) {
        # 存在する場合は読み込む
        foreach ($Line in [File]::ReadAllLines($DimScreenIniFilePath)) {
            $Line = $Line.Replace("`r`n", "").Replace("`r", "").Replace("`n", "")
            $DimScreenIniFileLines.Add($Line)
        }
    }
    if (-not $DimScreenIniFileLines.Contains("[Settings]")) {
        $DimScreenIniFileLines.Add("[Settings]")
    }
    if (-not $DimScreenIniFileLines.Contains("dimming=0")) {
        $DimScreenIniFileLines.Add("dimming=0")
    }
    [File]::WriteAllText($DimScreenIniFilePath, [String]::Join("`r`n", $DimScreenIniFileLines) + "`r`n")

    # 変数定義
    $DimScreenExecutablePath = $CheckTargetFilePath

    # ユーザーごとのスタートアップフォルダーにショートカットを作成
    $StartupFolderPath = [Environment]::GetFolderPath("Startup")
    $ShortcutName = "DimScreen"
    $ShortcutLinkFilePath = Join-Path $StartupFolderPath "$ShortcutName.lnk"
    # 存在しなければ新規に作成
    if (-not (Test-Path $ShortcutLinkFilePath -PathType Leaf)) {
        $ShortcutTargetPath = $DimScreenExecutablePath
        $ShortcutArguments = "0"
        $ShortcutDescription = "DimScreenの起動"
        $ShortcutIconPath = $DimScreenExecutablePath
        $ShortcutIconIndex = 0
